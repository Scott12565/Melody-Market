...
const Player = () => {
  ...
  const [hasPlayed, setHasPlayed] = useState(false); // Track if the song has been played
  const playerRef = useRef(null);

  useEffect(() => {
    if (isPlaying && !hasPlayed) {
      setIsCollapsed(false); // Expand if the song starts playing
      setHasPlayed(true);    // Mark as played to prevent repeated expansions
    }
  }, [isPlaying, hasPlayed]);

  useEffect(() => {
    if (playerRef.current) {
      playerRef.current.style.maxHeight = isCollapsed ? '3rem' : `${playerRef.current.scrollHeight}px`;
    }
  }, [isCollapsed]);
  ...
  const toggleCollapse = () => {
    if (!isPlaying) {
      setIsCollapsed(prev => !prev); // Toggle collapse state if the song is not playing
    }
  };

  return (
    <div
      className={`fixed bottom-0 left-0 z-50 w-screen p-1 px-3 bg-[#0E1411] opacity-[75] transition-all duration-300 ease-in-out`}
    ...
    >
      <div
        ...
        style={{
          maxHeight: isCollapsed ? '3rem' : `${playerRef.current?.scrollHeight}px`,
        }}
      >
